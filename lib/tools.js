 ...
 * @typedef {{name: string, optionKey: GLSLToolPathKeys, url: string, distPath?: string, path?: string}} GLSLSingleToolConfig
...
const toolInfo = (config) => `${config.name} : configure with ${config.optionKey}\n${config.url}\n`;
...
export function configureTools(options, required = /** @type {GLSLToolVals[]} */(Object.keys(ToolConfig))) {
  ...
  for (const kind of required) {
    ...
    const toolPath = options[tool.optionKey] || tool.distPath;
    if (!toolPath) {
